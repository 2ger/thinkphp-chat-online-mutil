...
class Manager extends Base
{
   ...
   public function replyinfo(){
      ...
      $res =Admins::table('wolive_reply')->where('service_id',$login['service_id'])->order('id desc')->limit(10)->select();
   ...
   public function delreply(){
    ...
    public function group(){
        ...
        $group =Admins::table('wolive_group')->where('business_id',$login['business_id'])->paginate(8);
        $page = $group->render();
    ...
    public function saveVisiter(){
        ...
        $res =Admins::table('wolive_visiter')->where('visiter_id',$post['visiter_id'])->update(['visiter_name'=>$post['visiter_name'],'connect'=>$post['connect'],'comment'=>$post['comment']]);
    ...
    public function addquestion()
    {
        ...
        $post['business_id']=$_SESSION['Msg']['business_id'];
         ...
         $post['answer']=trim($post['answer']);

         if (mb_strlen($post['keyword'],'UTF8') > 6) {
             $data =['code'=>1,'msg'=>'关键词不能大于6个字！'];
             return $data;
         }
        ...
        if(isset($post['qid'])){
            $res =Admins::table('wolive_question')->where('qid',$post['qid'])->update(['question'=>$post['question'],'answer'=>$post['answer'],'answer_read'=>$post['answer_read'],'keyword'=>$post['keyword'],'sort'=>$sort]);
